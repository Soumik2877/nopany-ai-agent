...
import React, { useState, useEffect } from 'react';
import { useLiveAPI } from './hooks/useLiveAPI';
import { openHand, closeHand } from './hooks/useHandAPI'; // Removed setEyeExpression
import AudioVisualizer from './components/AudioVisualizer';
import { Mic, MicOff, Phone, PhoneOff, AlertCircle, Info, GraduationCap } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Helper function for the greeting gesture (Close -> Wait -> Open)
  const performGreetingGesture = async () => {
    if (handBusy) return;
    
    try {
      setHandBusy(true);
      console.log("Performing greeting gesture...");
      
      // 1. Close Hand
      await closeHand();
      
      // 2. Wait for 500ms (half a second)
      ...
      console.error("Greeting gesture failed:", e);
      setHandError("Gesture failed");
    } finally {
      setHandBusy(false);
    }
  };
  ...
  const { connect, disconnect, isConnected, isConnecting, error, analyser } = useLiveAPI({
    onModelText: async (text: string) => {
      ...
      // If the AI says any of these words, trigger the hand wave/namaste gesture
      const greetingKeywords = ['hello', 'hi ', 'namaste', 'welcome', 'good morning', 'good afternoon'];
      const isGreeting = greetingKeywords.some(keyword => lower.includes(keyword));

      if (isGreeting && !handBusy) {
         performGreetingGesture();
         // We return here to avoid double-triggering if the greeting also contains "open hand" (unlikely)
         return; 
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900">
      ...
      <footer className="bg-white border-t border-slate-100 py-4 text-center text-slate-400 text-xs">
         <p>© 2026 Nopany High School. All rights reserved.</p>
         <p className="mt-1">Powered by India-Elevate@2026 Live API</p>
      </footer>
    </div>
  );
};
